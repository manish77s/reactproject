...
const Commoncompo = () => {
  ...
  return (
    ...
    {allcomp.filter((item) =>item.id===55).map((data,index)=>{
      ...
      return(
        <div key={index}>
          <Link to={`/details/${Food.heading}`} state={data}><h3>{data.heading}</h3><img className='topimage' src={data.image}/></Link>
    ...
    {allcomp.filter((item) =>item.id===22).map((data,index)=>{
      ...
      return(
        <div key={index}>
          <Link to={`/details/${Hollywood.heading}`} state={data}><img className='top4' src={data.image}/><h3>{data.heading}</h3></Link>
